...
class Dota(commands.Cog):
    ...
    @commands.command(brief='Stores all Discord IDs of current server into database')
    async def loadIDs(self, ctx):
    ...
    @commands.command(brief='Saves Steam ID to database. Uses steamID3 (same ID as opendota)')
    async def steamID(self, ctx, s='', name=''):
        if not s.isdigit():
    ...
    @commands.command(brief='Generates Dota 2 word cloud for user')
    async def wordcloud(self, ctx, name=''):
        if name == '':
    ...
    @commands.command(brief='Dota 2 win rate of user')
    async def wr(self, ctx, name=''):
        if name == '':
            s = sID.returnSteamID(ctx.author.id)
        else:
            s = sID.returnSteamID(sID.returnDiscordID(name))
    ...
    @commands.command(brief='Dota 2 averages of user')
    async def averages(self, ctx, name=''):
        if name == '':
            s = sID.returnSteamID(ctx.author.id)
        else:
            s = sID.returnSteamID(sID.returnDiscordID(name))
    ...
    @commands.command(brief='Stats from last Dota 2 match played by user')
    ...
    @commands.command(brief='Stats for Dota 2 game from given match ID')
    async def match(self, ctx, m=''):
        try:
            matchInfo = opendota.getMatch(m)
            await matchInfo.get_match_data()
    ...
    @commands.command(brief='Gives match IDs and heroes played of user. Default 5, can specify for more')
    async def matchIDs(self, ctx, limit=5, name=''):
        if name == '':
            s = sID.returnSteamID(ctx.author.id)
        else:
            s = sID.returnSteamID(sID.returnDiscordID(name))
    ...
    @commands.command(brief='Check Dota 2 subreddit vir latest update thread from SirBelvedere')
    async def update(self, ctx):
        r = await reddit.newUserPost('Dota2', 'SirBelvedere')
        await ctx.send(r[0] + '\n\n' + r[1])

    @commands.command(brief='Check for latest Dota 2 blog post')
