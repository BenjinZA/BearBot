...
async def get_live_games():
    session = aiohttp.ClientSession()
    live_data = await http_util.send_http(session, 'get', 'http://api.steampowered.com/IDOTA2Match_570/GetLiveLeagueGames/v1?key=39FAC0FFC4F9AFBCAE239813071B6BE0', retries=10)
...
async def check_if_still_live(match_id):
    session = aiohttp.ClientSession()
    match_data = await http_util.send_http(session, 'get', 'http://api.steampowered.com/IDOTA2Match_570/GetLiveLeagueGames/v1?key=39FAC0FFC4F9AFBCAE239813071B6BE0&match_id=%d' % match_id, retries=10)
    match = await match_data.json()
    session.close()
